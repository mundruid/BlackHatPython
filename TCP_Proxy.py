...
def proxy_handler(client_socket, remote_host, remote_port, receive_first):
    ...
    while True:
        ...
        if len(local_buffer):
            ...
            # send off the data to a remote host
...
# pretty hexdump function taken from here: http://code.activestate.com/recipes/142812-hex-dumper/
def hexdump(src, length=16):
    result = []
    digits = 4 if isinstance(src, UnicodeWarning) else 2
...
# modify any requests destined for the remote host, ex. fuzz, etc.
def request_handler(buffer):
    # perform packet modifications
    return buffer

# modify any responses destined for the local host, ex. check authenticity, etc.
...
def server_loop(local_host, local_port, remote_host, remote_port, receive_first):
    ...
    try:
        ...
        print("[!!] Check for other listening sockets of correct permissions.")
        sys.exit(0)
...
def main():
    ...
    # setup local listening parameters, sys.argv[0] is the name of the script
